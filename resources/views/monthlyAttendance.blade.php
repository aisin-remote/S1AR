                                ...
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        @php
                                            ...
                                            $jenis = match ($jumlahKarakter) {
                                                5 => 'KDP',
                                                7 => 'SPV',
                                                9 => 'LDR/OPR',
                                                2, 3 => 'GMR',
                                                default => 'Jenis tidak dikenali',
                                            };
                                        @endphp
                                        <td>{{ $jenis }}</td>
                                        ...
                                        @for ($hari = 1; $hari <= $jumlah_hari; $hari++)
                                            @php
                                                ...
                                                $today = date('j');
                                                $month = date('m');
                                                $date = sprintf('%04d-%02d-%02d', $tahunSekarang, $bulanSekarang, $hari);
                                                $dayOfWeek = (new DateTime("$tahunSekarang-$bulanSekarang-$hari"))->format('w');
                                                $isWeekend = $dayOfWeek == 0 || $dayOfWeek == 6;

                                                if ($bulanSekarang == $month && $hari <= $today) {
                                                    foreach ($npkData as $data) {
                                                        if (!is_null($data->schdt) && date('j', strtotime($data->schdt)) == $hari) {
                                                            ...
                                                            if (in_array(trim($rsccd), [
                                                                'HDR', 'TL1', 'TL2', 'TL3', 'HHO', 'BKT', 'DTA', 'HHL', 'OT>03', 'OT>11', 'PCT', 'PTA', 'SF2', 'SF3', 'SLP', 'TL1', 'TL2', 'TL3', 'OVDAY', 'OVHOL', 'OVSPD', 'DLU', 'TRNT', 'TRT',
                                                            ])) {
                                                                $hdrCount++;
                                                            }
                                                ...
                                                // If the current day is a weekend and rsccd is not set and day is before today, assign "HOF"
                                                if ($rsccd == '' && $isWeekend && $hari <= $today) {
                                                    $rsccd = 'HOF';
                                                }
                                            @endphp

                                            <td {!! in_array(trim($rsccd), ['HDR', 'TL1', 'TL2', 'TL3', 'SF2', 'SF3']) ? 'class="text-success text-center"' : 'class="text-center"' !!}>
                                                @if (in_array(trim($rsccd), ['HDR', 'TL1', 'TL2', 'TL3', 'SF2', 'SF3']))
                                                    ✔️
                                                @else
                                                    {!! in_array(trim($rsccd), ['ALP']) ? '<span class="badge badge-danger">' . $rsccd . '</span>' : ($rsccd ? '<span class="badge badge-warning">' . $rsccd . '</span>' : '') !!}
                                                @endif
                                            </td>
                                        @endfor
